...
Since significant discussion has already occurred surrounding branch, this will be short. ``branch`` can accept a string as input and it enqueues all incoming results into the named queue. If instead it is given an array of names, it will enqueue a copy into each listed queue. Once the queue ``branch`` is pulling from closes, it *will NOT* close the queue(s) it has been pushing values into. Since they are named queues and not directly tied to each other anonymously, more than one queue could be feeding results into it. If you want to close that queue, do the following:
...
This is the most complex method for ``queue-flow``. ``node`` has traits of ``map``, ``filter``, and ``branch`` grafted together with a special behavior for values that are arrays to work particularly well with Node.js-style APIs. Specifically APIs that follow this convention:
